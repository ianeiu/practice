package com.wm.demo.activiti.d2variables;
...
//		taskService.complete(taskId, variables)//完成任务的同时，设置流程变量，用Map集合
		...
		/**获取流程变量*/
		System.out.println(runtimeService.getVariable(executionId, "人员信息(添加固定版本)"));//使用执行对象ID和流程变量的名称，获取流程变量的值
		System.out.println(runtimeService.getVariables(executionId));//使用执行对象ID，获取所有的流程变量，将流程变量放置到Map集合中，map集合的key就是流程变量的名称，map集合的value就是流程变量的值
		//runtimeService.getVariables(executionId, variableNames);//使用执行对象ID，获取流程变量的值，通过设置流程变量的名称存放到集合中，获取指定流程变量名称的流程变量的值，值存放到Map集合中
		
		System.out.println(taskService.getVariable(taskId, "人员信息(添加固定版本)"));//使用任务ID和流程变量的名称，获取流程变量的值
		System.out.println(taskService.getVariables(taskId));//使用任务ID，获取所有的流程变量，将流程变量放置到Map集合中，map集合的key就是流程变量的名称，map集合的value就是流程变量的值
